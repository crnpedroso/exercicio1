...
if [ -z "$JDK" ] && [ -s "$HOME/.PyCharmCE2019.2/config/pycharm.jdk" ]; then
  USER_JRE=$("$CAT" "$HOME/.PyCharmCE2019.2/config/pycharm.jdk")
  if [ ! -d "$USER_JRE" ]; then
...
elif [ -r "$HOME/.PyCharmCE2019.2/config/pycharm$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="$HOME/.PyCharmCE2019.2/config/pycharm$BITS.vmoptions"
...
"$JAVA_BIN" \
  ...
  -Didea.paths.selector=PyCharmCE2019.2 \
